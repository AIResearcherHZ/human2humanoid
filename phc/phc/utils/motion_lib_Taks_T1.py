"""
Motion Library for Taks_T1 robot (32 DOF humanoid with neck and wrist joints)

Based on motion_lib_h1.py, adapted for Taks_T1 robot structure:
- Left/Right Leg: 6 DOF each (hip pitch/roll/yaw, knee, ankle pitch/roll)
- Waist: 3 DOF (yaw, roll, pitch)
- Left/Right Arm: 7 DOF each (shoulder pitch/roll/yaw, elbow, wrist roll/yaw/pitch)
- Neck: 3 DOF (yaw, roll, pitch)
"""
...
from phc.utils.motion_lib_base import MotionLibBase, DeviceCache, FixHeightMode
from phc.utils.torch_Taks_T1_humanoid_batch import Humanoid_Batch_Taks_T1
...
class MotionLibTaksT1(MotionLibBase):
    """
    Motion Library for Taks_T1 robot.
    
    Taks_T1 has 32 DOF:
    - Left Leg: 6 DOF (hip pitch/roll/yaw, knee, ankle pitch/roll)
    - Right Leg: 6 DOF (hip pitch/roll/yaw, knee, ankle pitch/roll)
    - Waist: 3 DOF (yaw, roll, pitch)
    - Left Arm: 7 DOF (shoulder pitch/roll/yaw, elbow, wrist roll/yaw/pitch)
    - Right Arm: 7 DOF (shoulder pitch/roll/yaw, elbow, wrist roll/yaw/pitch)
    - Neck: 3 DOF (yaw, roll, pitch)
    """

    def __init__(self, motion_file, device, fix_height=FixHeightMode.no_fix, masterfoot_conifg=None, 
                 min_length=-1, im_eval=False, multi_thread=True, extend_hand=True, extend_head=False, 
                 mjcf_file="resources/robots/Taks_T1/Taks_T1.xml", sim_timestep=1/50):
        ...
        self.mesh_parsers = Humanoid_Batch_Taks_T1(extend_hand=extend_hand, extend_head=extend_head, mjcf_file=mjcf_file)
        return
    ...
    def fix_trans_height(pose_aa, trans, curr_gender_betas, mesh_parsers, fix_height_mode):
        ...
        with torch.no_grad():
            raise NotImplementedError("Fix height is not implemented for Taks_T1")
    ...
    def load_motions(self, skeleton_trees, gender_betas, limb_weights, random_sample=True, start_idx=0, max_len=-1, target_heading=None):
        """
        Load motions for Taks_T1 robot.
        
        Args:
            skeleton_trees: List of skeleton trees for each humanoid
            gender_betas: Gender and body shape parameters
            limb_weights: Weights for different limbs
            random_sample: Whether to randomly sample motions
            start_idx: Starting index for sequential sampling
            max_len: Maximum length of motion to load (-1 for full motion)
            target_heading: Target heading direction
        """
        motions = []
        _motion_lengths = []
        _motion_fps = []
        ...
        jobs = [(ids[i:i + chunk], jobs[i:i + chunk], skeleton_trees[i:i + chunk], gender_betas[i:i + chunk], 
                 self.fix_height, self.mesh_parsers, self._masterfoot_conifg, target_heading, max_len) 
    ...
    def get_motion_state(self, motion_ids, motion_times, offset=None):
        """
        Get the motion state at specified times.
        
        Args:
            motion_ids: Motion IDs to query
            motion_times: Times within each motion
            offset: Optional position offset
            
        Returns:
            Dictionary containing motion state (positions, rotations, velocities, etc.)
        """
        n = len(motion_ids)
        ...
        if "dof_pos" in self.__dict__:  # Taks_T1 joints
    ...
    def load_motion_with_skeleton(ids, motion_data_list, skeleton_trees, gender_betas, fix_height, mesh_parsers, masterfoot_config, target_heading, max_len, queue, pid):
        """
        Load motion data with skeleton information.
        
        Performs forward kinematics to compute joint positions from motion data.
        """
